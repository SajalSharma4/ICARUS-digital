<?php 
  ...
  if(isset($_POST['cart'])){
    ...
    $sql_query = "SELECT id as userid FROM users WHERE email = '$email';";
    $result = mysqli_query($conn,$sql_query);
    $row = mysqli_fetch_array($result);
    $user_id=$row['id'];
    $sql_query2 = "INSERT INTO cart (user_id, product_id) VALUES ( '$user_id', '$product_id')";
    $resul2 = mysqli_query($conn,$sql_query2);
...
<body>
    ...
    <div id="web-content" method="post">
        <section id="products"> 
          <div class="row row-cols-1 m-5 row-cols-md-3 g-4">
            <?php 
            $sql_query = "SELECT product_id,name,img,price,demand,date from product order by date DESC;";
            $result = mysqli_query($conn,$sql_query);
            while($data=mysqli_fetch_array($result)) {
            ?>
              <div class="col">
                  <div class="card h-100">
                    <img  src="../assets/images/products/<?php echo $data['img']; ?>" class="card-img-top" alt="<?php echo $data['name'].' img'; ?>">
                    <div class="card-body">
                      <h3 class="card-title" style="color: black; text-decoration:underline"><?php echo $data['name']?></h3>                   
                    </div>
                    <div>
                    <h4 class="text-muted" style="text-align: center; margin-bottom:20%; margin-top:0%" ><?php echo 'Rs.'.$data['price']; ?></h4>
                    <h4 class="text-muted" style="text-align: center; margin-bottom:20%; margin-top:0%" ><?php echo 'Bought By '.$data['demand'].' users'; ?></h4>
                    </div>
                    <a href="../pages/cart.php" type= "button" class="btn btn-primary" name="cart" <?php $product=$data['product_id']?>><?php echo $cart_message?></a>
                  </div>
        ...
        <div class="footer-left">
          <ul class="nav flex-column">
            ...
            <li class="nav-item">
              <a class="nav-link last" href="review.php">FAQ's</a>
            </li>
          </ul>
      </div>
