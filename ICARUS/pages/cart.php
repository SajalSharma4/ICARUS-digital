      ...
      <div id="web-content" method="post">
        <section id="products"> 
          <div class="row row-cols-1 m-5 row-cols-md-3 g-4">
            ...
            $sql_query2 = "SELECT id as user_id FROM users WHERE email = '$email';";
            $result2 = mysqli_query($conn,$sql_query2);
            $row2 = mysqli_fetch_array($result2);
            $user_id=$row2['user_id'];

            $sql_query = "SELECT product.product_id,product.name,img,price,demand from product INNER JOIN (cart INNER JOIN users ON cart.product_id=users.id) ON cart.product_id=product.product_id where cart.user_id='$user_id' order by product.date DESC;";
