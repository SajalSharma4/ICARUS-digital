...
if(isset($_POST['changepwd'])){
  ...
  if( $name!="" && $contact!="" && $npassword!="" && $cpassword!="" ) {
    $sql_query="SELECT contact as usercontact FROM users WHERE name = '$name'";
    $result = mysqli_query($conn,$sql_query);
    $row = mysqli_fetch_array($result);
    $usercontact = $row['usercontact'];
    if($contact===$usercontact){
        if($npassword===$cpassword){
            $pwdencrypt = password_hash($npwd,PASSWORD_DEFAULT);
            $sql_query="UPDATE users SET password='$pwdencrypt' WHERE name='$name'";
            $result = mysqli_query($conn,$sql_query);
            if($result){
                session_destroy();
        ...
        $message="Username and contact number do not match";
    }
  }
  else{
        ...
        <div class="container-fluid">
          ...
          <div class="collapse navbar-collapse" id="navbarSupportedContent">
            <ul class="navbar-nav me-auto mb-2 mb-lg-0">
              ...
              <li class="nav-item">login.php">Subscribe and Save</a>
              </li>
            ...
            <?php } else {?>
              <a class="btn btn-outline-success" href="login.php" type="button" title="Cart"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-cart" viewBox="0 0 16 16">
                <path d="M0 1.5A.5.5 0 0 1 .5 1H2a.5.5 0 0 1 .485.379L2.89 3H14.5a.5.5 0 0 1 .491.592l-1.5 8A.5.5 0 0 1 13 12H4a.5.5 0 0 1-.491-.408L2.01 3.607 1.61 2H.5a.5.5 0 0 1-.5-.5zM3.102 4l1.313 7h8.17l1.313-7H3.102zM5 12a2 2 0 1 0 0 4 2 2 0 0 0 0-4zm7 0a2 2 0 1 0 0 4 2 2 0 0 0 0-4zm-7 1a1 1 0 1 1 0 2 1 1 0 0 1 0-2zm7 0a1 1 0 1 1 0 2 1 1 0 0 1 0-2z"/>
              </svg></a>
            <?php } ?>
              
              <?php if(isset($_SESSION['username'])) { ?>
              <a class="btn btn-outline-success" href="user_profile.php" type="button" title=<?php echo $_SESSION['username']?>><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-person-circle" viewBox="0 0 16 16">
                <path d="M11 6a3 3 0 1 1-6 0 3 3 0 0 1 6 0z"/>
                <path fill-rule="evenodd" d="M0 8a8 8 0 1 1 16 0A8 8 0 0 1 0 8zm8-7a7 7 0 0 0-5.468 11.37C3.242 11.226 4.805 10 8 10s4.757 1.225 5.468 2.37A7 7 0 0 0 8 1z"/>
              </svg></a>
            <?php } else {?>
              <a class="btn btn-outline-success" href="login.php" type="button" title="Profile"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-person-circle" viewBox="0 0 16 16">
                <path d="M11 6a3 3 0 1 1-6 0 3 3 0 0 1 6 0z"/>
                <path fill-rule="evenodd" d="M0 8a8 8 0 1 1 16 0A8 8 0 0 1 0 8zm8-7a7 7 0 0 0-5.468 11.37C3.242 11.226 4.805 10 8 10s4.757 1.225 5.468 2.37A7 7 0 0 0 8 1z"/>
              </svg></a>
            <?php } ?>
  ...
  <form action="" method="post">
    ...
    <div class="content">
      <div class="input-field">
        <input type="text" placeholder="Enter Your Name" autocomplete="nope" name="name">
      </div>
      <div class="input-field">
        <input type="tel" placeholder="Enter Contact No." autocomplete="new-password" name="contact">
      </div>
      <div class="input-field hidden">
        <input type="password" placeholder="Enter New Password" autocomplete="new-password" name="npassword">
      </div>
      <div class="input-field hidden">
        <input type="password" placeholder="Confirm New Password" autocomplete="new-password" name="cpassword">
      </div>
    </div>
    <div class="action">
    <button name="submit" style="display: none;">Sign in</button>
      <button name="changepwd">Change Password</button>
    </div>
  </form>
</div>
